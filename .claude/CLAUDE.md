...
- Start independent read-only tasks in parallel and prefer background runs when their results are not immediately blocking.
...
- For non-trivial implementation, use the matching flow rather than invoking every role mechanically: `Explore` and/or a research agent → `Plan` when design is needed → one implementation agent → a separate fresh-context review agent.
- Prefer specialized installed agents when their descriptions match; otherwise use `general-purpose` with a focused assignment.
